...
def plot_bar(title, womenMeans, languages, norm=False, ylabel='Время выполнения', units='с'):
    ...
    if norm:
        womenMeans = [item / m for item in womenMeans]
    ...
    rects2 = ax.bar(ind + width, womenMeans, width,
                    ...
                    path_effects=[path_effects.SimpleLineShadow(),
                                  path_effects.Normal()], align='center'
                    )
    ...
    if norm:
        s = '%s по отношению к ' % ylabel + languages[0];
    ...
    def autolabel(rects):
        ...
        for i, rect in enumerate(rects):
            ...
            if int(height) == height:
                ...
                if norm:
                    text_height = "%d" % (height * 100) + "%"
                else:
                    text_height = "%.3f" % height

            text = ax.text(rect.get_x() + rect.get_width() / 2., height * 0.0,
                           text_height,
                           ha='center', va='bottom', rotation='vertical', color='black', weight='bold',
                           size=16,
